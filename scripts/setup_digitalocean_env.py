...
def create_production_env():
    ...
    print("The DigitalOcean database connection is pre-configured.")
    print("Please provide the following information:\n")
    
    allowed_origins = input("Enter allowed CORS origins (comma-separated, e.g., https://example.com): ").strip()
    if not allowed_origins:
        allowed_origins = "https://your-domain.com,https://www.your-domain.com"
    
    vite_api_base_url = input("Enter your API base URL (e.g., https://api.example.com): ").strip()
    if not vite_api_base_url:
        vite_api_base_url = "https://your-domain.com"
    
    openai_api_key = input("Enter your OpenAI API key (optional, press Enter to skip): ").strip()
    if not openai_api_key:
        openai_api_key = "your_openai_api_key_here"
    
    redis_password = input("Enter Redis password (or press Enter to use default): ").strip()
    ...
    try:
        ...
        print("   - DigitalOcean PostgreSQL database (VPC connection)")
        ...
        print("\n📝 Next steps:")
        print("   1. Review the .env file and update any remaining values")
        print("   2. Ensure your app can access the DigitalOcean VPC network")
        print("   3. Run database migrations: alembic upgrade head")
        print("   4. Create an admin user: python create_admin.py")
