            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Hello {user?.full_name || user?.email}! {useAGUI ? (useCopilotKit ? 'AG-UI + CopilotKit Enhanced Chat' : 'AG-UI powered chat') : 'Traditional chat interface'}
                </p>
              </div>
              <div className="flex items-center space-x-4">
                ...
                {useAGUI && (
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-gray-600">CopilotKit</span>
                    <button
                      onClick={() => setUseCopilotKit(!useCopilotKit)}
                      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                        useCopilotKit ? 'bg-blue-600' : 'bg-gray-200'
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          useCopilotKit ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
